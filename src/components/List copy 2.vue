<template>
  <ul
    ...
    <li
      ...
      @mouseleave="mouseEnter=false;mouseIndex=-1;aa(index)">
      <slot
        name="item"
        :index="index"
        :item="item"
...
export default {
  ...
  watch: {
    ...
    mouseEnter: function (newVal, oldVal) {
      console.log('List:mouseEnter', newVal, oldVal)
    },
  },
  methods: {
    aa(index) {
      ...
      // self.mouseStart = true;
        ...
        if(self.w.t2 == self.w.t1){
          if(e.target.scrollTop%self.itemHeight != 0) {
            ...
            console.log('scroll-finish1', self.mouseEnter);
